...
public class CommandGetChest implements TabExecutor {
    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(commandSender instanceof Player player){
            if(strings.length == 1){
                ...
                if(player == target){
                    player.sendMessage(MessageMaker.makeMessage(Text.getText(Text.admin_command_chest_senderIsTarget), TextTypes.NO_SUCCESS));
                    return true;
                }
                if(s.equals("chest")){
                    player.openInventory(target.getInventory());
                } else if(s.equals("echest")){
